...
- 支持从 Clash API 拉取策略组列表并下拉选择监控组
- 支持中文界面和中文通知文案
- 设置窗口支持置顶显示
- 手动检查后显示临时结果提示条
- Clash API Secret 支持小眼睛切换明文/密文
...
- 如果策略组列表没有及时出现，可以点击“刷新策略组列表”立即重试
- Clash API 的 Secret 输入框右侧有小眼睛按钮，可切换明文或隐藏显示
...
- 如果 Clash API 已返回可识别的策略组，设置页会把这些组名列出来，并提供下拉选择
- 打开设置页时，应用会主动刷新一次 Clash 策略组列表
- 如果你怀疑列表没刷新，可以手动点击“刷新策略组列表”
...
- Clash API Secret：支持点击小眼睛切换为明文查看，方便核对配置
...
- 增加手动检查后的临时提示条
- 设置页改为更稳定的滚动卡片布局，避免部分区域显示为空白
- Clash API 设置增加策略组列表刷新和下拉选择
- Clash API Secret 增加明文切换按钮
